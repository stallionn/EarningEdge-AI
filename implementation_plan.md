...
> No shortcuts. No placeholders. This is the exact execution roadmap to deliver best-in-class results.
...
1. **Data Ingestion Layer** — Collect & preprocess earnings call transcripts + price data
2. **NLP Model Layer** — Fine-tuned FinBERT + hedge NER + management confidence scoring
3. **Alpha Signal Layer** — Cross-reference sentiment scores with price returns, backtest, REST API
...
- [ ] **Recommended hybrid approach**:
  1. Start with `EntityRuler` (rule-based, instant, high precision)
  2. Train statistical NER on top for novel phrase generalization
...
- [ ] Pipeline on request:
  1. Preprocess text (speaker diarization, sentence segmentation)
  2. Run fine-tuned FinBERT inference (batch sentences)
  3. Run hedge NER
  4. Compute confidence score
  5. Compute alpha signal composite
